...
func _open_chest() -> void:
	...
	if main_game:
		...
		# Добавляем звёздную карту в инвентарь
		inventory.add_item("starmap")
		# Выдвигаем инвентарь если скрыт
		if not inventory.is_open:
			inventory._on_toggle_button_pressed()

	# Даём игроку время увидеть предмет в инвентаре перед закрытием
