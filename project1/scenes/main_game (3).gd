...
var intro_finished: bool = false  # флаг — вступление завершено

func _ready() -> void:
	fade_rect.modulate.a = 1.0  # начинаем с чёрного экрана
	_load_room(room_index)
	_set_room1_arrows_visible(false)  # скрываем стрелки на время вступления
	_play_wakeup_blink()
...
func _on_intro_dialog_finished() -> void:
	intro_finished = true
	_set_room1_arrows_visible(true)  # показываем стрелки после диалога
