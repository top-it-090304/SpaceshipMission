...
# Сообщения после стирания пыли (загадка)
# TODO: замени текст загадки на свой
var messages_riddle: Array[String] = [
	"Загадка будет здесь.",
]
...
func _ready() -> void:
	...
	# Если пыль уже стёрта — сразу показываем шар чистым
...
func _on_ball_pressed() -> void:
	...
	if not GameState.ball_cleaned and main_game != null:
		...
		if inventory.get_selected_item_id() == "brush":
			...
			dust_layer.visible = false
			_open_dialog(messages_riddle)
			return

	# Пыль уже стёрта — показываем загадку
	if GameState.ball_cleaned:
		_open_dialog(messages_riddle)
	else:
		# Brush не выбрана — показываем «пыльный»
