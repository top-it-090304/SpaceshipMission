...
var screen_unlocked: bool = false  # подтверждён ли доступ к панели управления
...
func _ready() -> void:
	...
	if GameState.intro_finished:
		# Повторный заход — без моргания и диалога
		fade_rect.modulate.a = 0.0
		input_blocker.visible = false
		_load_room(room_index)
	else:
		# Первый запуск — моргание и диалог
		fade_rect.modulate.a = 1.0
		_load_room(room_index)
		_set_room1_arrows_visible(false)
		$UILayer/InventoryRoot/ToggleButton.visible = false
		input_blocker.visible = true  # блокируем клики на комнату
		_play_wakeup_blink()
		dialog_box.dialog_finished.connect(_on_intro_dialog_finished)

# --- Эффект пробуждения (только при старте) ---
func _play_wakeup_blink() -> void:
	var tween := create_tween()
	tween.set_ease(Tween.EASE_IN_OUT)
	tween.set_trans(Tween.TRANS_SINE)

	# Моргания: [время_темно, время_светло] — сначала частые, потом редкие
	var blinks := [
		[0.15, 0.10],   # быстро мигнул
		[0.18, 0.10],   # ещё раз
		[0.25, 0.15],   # чуть медленнее
		[0.40, 0.25],   # почти открыл
		[0.60, 0.45],   # долго открыт
		[0.80, 0.0],    # последнее — остаётся открытым
	]

	for blink in blinks:
		var close_time: float = blink[0]
		var open_time: float  = blink[1]
		# темнеем (закрываем глаза)
		tween.tween_property(fade_rect, "modulate:a", 1.0, close_time)
		# светлеем (открываем глаза)
		if open_time > 0.0:
			tween.tween_property(fade_rect, "modulate:a", 0.0, open_time)

	# В конце остаёмся с открытым экраном
	tween.tween_property(fade_rect, "modulate:a", 0.0, 0.5)
	# После моргания запускаем диалог
...
func _on_intro_dialog_finished() -> void:
	...
	input_blocker.visible = false  # разблокируем клики
