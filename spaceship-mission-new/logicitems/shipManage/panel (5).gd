...
# Совпадают с ScreenBg: position (369, 147), size (546 x 411)
const SCREEN_LEFT: float = 369.0
const SCREEN_TOP: float = 147.0
const SCREEN_WIDTH: float = 546.0
const SCREEN_HEIGHT: float = 411.0

const ARROW_SPEED: float = 220.0
const DOT_SPEED_START: float = 160.0
...
# --- Ноды первой игры ---
@onready var left_screen: ColorRect = $Panel/LeftScreen
@onready var left_label: Label = $Panel/LeftLabel
@onready var left_plus: TextureButton = $Panel/LeftPlus
@onready var left_minus: TextureButton = $Panel/LeftMinus
@onready var right_screen: ColorRect = $Panel/RightScreen
@onready var right_label: Label = $Panel/RightLabel
@onready var right_plus: TextureButton = $Panel/RightPlus
@onready var right_minus: TextureButton = $Panel/RightMinus
@onready var back_button: TextureButton = $Panel/BackButton

# --- Ноды второй игры ---
@onready var joystick: Node2D = $Panel/Joystick
@onready var joy_up: TextureButton = $Panel/Joystick/JoyUp
@onready var joy_down: TextureButton = $Panel/Joystick/JoyDown
@onready var joy_left: TextureButton = $Panel/Joystick/JoyLeft
@onready var joy_right: TextureButton = $Panel/Joystick/JoyRight
@onready var screen_area: Node2D = $Panel/ScreenArea
@onready var screen_bg: ColorRect = $Panel/ScreenArea/ScreenBg
@onready var arrow: Sprite2D = $Panel/ScreenArea/Arrow
@onready var dot: Sprite2D = $Panel/ScreenArea/Dot
@onready var grid: Node2D = $Panel/ScreenArea/Grid
...
func _process(delta: float) -> void:
	...
	# Хаотичное движение — меняет направление случайно
	chaos_timer -= delta
	if chaos_timer <= 0.0:
		dot_dir = Vector2(randf_range(-1, 1), randf_range(-1, 1)).normalized()
		chaos_timer = randf_range(0.2, 0.5)
